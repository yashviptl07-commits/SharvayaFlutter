...
class SalesOrderBloc extends Bloc<SalesOrderEvents, SalesOrderStates> {
  ...
  Stream<SalesOrderStates> mapEventToState(SalesOrderEvents event) async* {
    ...
    if (event is SalesTargetDeleteCallEvent) {
      yield* _mapSalesTargetDeleteCallEventToState(event);
    }
    if (event is SalesTargetAddUpdateCallEvent) {
      yield* _mapSalesTargetAddUpdateCallEventToState(event);
    }
    ...
    if (event is SalesOrderApprovalListRequestForDashBoardEvent) {
      yield* _mapSalesOrderApprovalListRequestForDashBoardEventToState(event);
    }
  ...
  Stream<SalesOrderStates> _map_bankDetailsWithDialogEvent_state(
  ...
  Stream<SalesOrderStates> _mapMultiNoToProductDetailsRequestEventState1(
...
/*  Stream<SalesOrderStates> _map_fcm_notificationEvent_state(
  ...
  Stream<SalesOrderStates> _map_fcm_notificationEvent_new_state(
  ...
  Stream<SalesOrderStates> _mapGetPoProductListEventState(
      GetQuotationProductListEvent event) async* {
    try {
      baseBloc.emit(ShowProgressIndicatorState(true));
  ...
  Stream<SalesOrderStates> _map_getInsertProductEventState(
...
*/
  ...
  Stream<SalesOrderStates> _mapPoOneProductDeleteEventState(
  ...
  Stream<SalesOrderStates> _mapSalesTargetDeleteCallEventToState(
      SalesTargetDeleteCallEvent event) async* {
    try {
      baseBloc.emit(ShowProgressIndicatorState(true));
      String response = await userRepository
          .getSalesTargetDeleteApi(event.salesTargetDeleteRequest);
      yield SalesTargetDeleteResponseState(response);
    } catch (error, stacktrace) {
      baseBloc.emit(ApiCallFailureState(error));
      print(stacktrace);
    } finally {
  ...
  Stream<SalesOrderStates> _mapSalesTargetAddUpdateCallEventToState(
      SalesTargetAddUpdateCallEvent event) async* {
    try {
      baseBloc.emit(ShowProgressIndicatorState(true));
      SalesTargetAddUpdateResponse response = await userRepository
          .getSalesTargetAddUpdateAPI(event.salesTargetAddUpdateRequest);
      yield SalesTargetAddUpdateResponseState(response);
    } catch (error, stacktrace) {
      baseBloc.emit(ApiCallFailureState(error));
      print(stacktrace);
    } finally {
  ...
  Stream<SalesOrderStates> _mapSOShipmentListRequestEventToState(
    ...
    try {
      ...
      yield SOShipmentlistResponseState(response, event.salesOrderDetails,
          soExportListResponse, event.isCopy);
    } catch (error, stacktrace) {
      baseBloc.emit(ApiCallFailureState(error));
      print(stacktrace);
    } finally {
  ...
  Stream<SalesOrderStates> _mapSalesOrderApprovalListRequestEventToState(
  ...
  Stream<SalesOrderStates>
      _mapSalesOrderApprovalListRequestForDashBoardEventToState(
          SalesOrderApprovalListRequestForDashBoardEvent event) async* {
    try {
      baseBloc.emit(ShowProgressIndicatorState(true));
      SalesOrderListResponse response =
          await userRepository.getSalesOrderApprovalListForDashBoardAPI(
              event.salesOrderApprovalListRequest);
      yield SalesOrderApprovalListResponseForDashBoardState(
          response, int.parse(event.salesOrderApprovalListRequest.PageNo));
    } catch (error, stacktrace) {
      baseBloc.emit(ApiCallFailureState(error));
      print(stacktrace);
  ...
  Stream<SalesOrderStates> _mapSalesOrderApprovalSaveRequestEventToState(
  ...
  Stream<SalesOrderStates> _mapSalesOrderApprovalStatusListRequestEventToState(
    ...
    try {
      ...
      final Exception wrappedError =
          error is Exception ? error : Exception(error.toString());
      baseBloc.emit(ApiCallFailureState(wrappedError));
      print(stacktrace);
    } finally {
      baseBloc.emit(ShowProgressIndicatorState(false));
    }
  ...
  Stream<SalesOrderStates>
      _mapSalesOrderApprovalStatusListRequestForDashBoardEventToState(
          SalesOrderApprovalStatusListRequestEvent event) async* {
    try {
      baseBloc.emit(ShowProgressIndicatorState(true));
      SalesOrderListResponse response =
          await userRepository.getSalesOrderApprovalStatusListForDashBoardAPI(
              event.salesOrderApprovalStatusListRequest);
      yield SalesOrderApprovalStatusListForDashBoardResponseState(response);
    } catch (error, stacktrace) {
      final Exception wrappedError =
          error is Exception ? error : Exception(error.toString());
      baseBloc.emit(ApiCallFailureState(wrappedError));
  ...
  Stream<SalesOrderStates> _mapSalesOrderAddressDropDownRequestEventToState(
  ...
  Stream<SalesOrderStates> _mapSalesOrderAddressOrgDropDownRequestEventToState(
  ...
  Stream<SalesOrderStates> _mapSalesOrderNoToProductListCallEventToState(
    ...
    try {
      ...
      if (response.details.length != 0) {
        ...
        for (var i = 0; i < response.details.length; i++) {
          ...
          String Docref = response.details[i].docRefNo;
  ...
  Stream<SalesOrderStates> _mapSoKindAttListCallEventToState(
  ...
  Stream<SalesOrderStates> _mapCountryListCallEventToState(
