...
class DPurchaseOtherCharge_screenArguments {
  int StateCode;
  String HeaderDiscFromAddEditScreen;
  QuotationDetails editModel;

  DPurchaseOtherCharge_screenArguments(
      this.StateCode, this.editModel, this.HeaderDiscFromAddEditScreen);
}

class DPurchaseOtherCharge_screen extends BaseStatefulWidget {
  static const routeName = '/DPurchaseOtherCharge_screen';
  final DPurchaseOtherCharge_screenArguments arguments;

  DPurchaseOtherCharge_screen(this.arguments);
  @override
  BaseState<DPurchaseOtherCharge_screen> createState() =>
      _DPurchaseOtherCharge_screenState();
}

class _DPurchaseOtherCharge_screenState
    extends BaseState<DPurchaseOtherCharge_screen>
  ...
  void initState() {
    ...
    _dealerBloc.add(OtherChargeCallEvent(
      ...
      child: BlocConsumer<DealerBloc, DealerStates>(
        builder: (BuildContext context, DealerStates state) {
          if (state is GetDealerPurchaseProductState) {
            getProductFromDBResponse(state);
          }
          if (state is OtherChargeListResponseState) {
        ...
        buildWhen: (oldState, currentState) {
          if (currentState is GetDealerPurchaseProductState ||
              currentState is OtherChargeListResponseState) {
            return true;
          }
          return false;
        },
  ...
  Widget buildBody(BuildContext context) {
    return WillPopScope(
      ...
      child: Column(
        children: [
          getCommonAppBar(context, baseTheme, "Other Charges",
  ...
  void _onOtherChargeListResponse(OtherChargeListResponseState state) {
  ...
  void getProductFromDBResponse(GetDealerPurchaseProductState state) {
    _inquiryProductList.clear();

    for (int i = 0; i < state.dealerPurchaseProductList.length; i++) {
      _inquiryProductList.add(state.dealerPurchaseProductList[i]);
    }
  }
  ...
  void UpdateAfterHeaderDiscount() async {
    ...
    for (int i = 0; i < _inquiryProductList.length; i++) {
      if (_inquiryProductList[i].TaxType == 1) {
        ...
        InTotalNetAmnt += InclusiveNetAmntAfterHeaderDisAmnt;
      }
