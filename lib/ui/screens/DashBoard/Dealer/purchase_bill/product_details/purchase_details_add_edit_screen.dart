...
class AddDPurchaseProductAddEdit_screenArguments {
  DealerPurchaseProductDBTable model;
  int StateCode;
  String HeaderDiscAmnt;

  AddDPurchaseProductAddEdit_screenArguments(
      this.model, this.StateCode, this.HeaderDiscAmnt);
}

class DPurchaseProductAddEdit_screen extends BaseStatefulWidget {
  static const routeName = '/DPurchaseProductAddEdit_screen';
  final AddDPurchaseProductAddEdit_screenArguments arguments;

  DPurchaseProductAddEdit_screen(this.arguments);
  @override
  BaseState<DPurchaseProductAddEdit_screen> createState() =>
      _DPurchaseProductAddEdit_screenState();
}

class _DPurchaseProductAddEdit_screenState
    extends BaseState<DPurchaseProductAddEdit_screen>
  ...
  List<DealerPurchaseProductDBTable> _inquiryProductList = [];
  List<DealerPurchaseProductDBTable> _TempinquiryProductList = [];
  ...
  List<DealerPurchaseProductDBTable> temp = [];
    ...
    return BlocProvider(
      ...
      child: BlocConsumer<DealerBloc, DealerStates>(
        ...
        listener: (BuildContext context, DealerStates state) {
          if (state is InsertDealerPurchaseProductState) {
            insertResponseFromDB(state);
          }
          if (state is UpdateDealerPurchaseProductState) {
            UpdateResponseFromDB(state);
          }
          return super.build(context);
        },
        listenWhen: (oldState, currentState) {
          if (currentState is InsertDealerPurchaseProductState ||
              currentState is UpdateDealerPurchaseProductState) {
            return true;
          }
          return false;
        },
  ...
  void getProductFromDB(GetDealerPurchaseProductState state) {
    ...
    for (int i = 0; i < state.dealerPurchaseProductList.length; i++) {
      temp.add(state.dealerPurchaseProductList[i]);
    }
  }
  ...
  _onTapOfAdd() async {
    ...
    if (isProductExist == false) {
      if (isForUpdate) {
        _dealerBloc.add(UpdateDealerPurchaseProductDBTableCallEvent(
        ...
        _dealerBloc.add(InsertDealerPurchaseProductDBTableCallEvent(
            DealerPurchaseProductDBTable(
                "",
                "",
                productID,
  ...
  void insertResponseFromDB(InsertDealerPurchaseProductState state) {
    showCommonDialogWithSingleOption(context, state.response,
        positiveButtonTitle: "OK");
  }

  void UpdateResponseFromDB(UpdateDealerPurchaseProductState state) {
    showCommonDialogWithSingleOption(context, state.response,
        positiveButtonTitle: "OK");
  }
}
