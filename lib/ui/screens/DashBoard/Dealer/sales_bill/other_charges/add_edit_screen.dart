...
class DSaleBillOtherChargeScreenArguments {
  int StateCode;
  String HeaderDiscFromAddEditScreen;
  QuotationDetails editModel;

  DSaleBillOtherChargeScreenArguments(
      this.StateCode, this.editModel, this.HeaderDiscFromAddEditScreen);
}

class DSaleBillOtherChargeScreen extends BaseStatefulWidget {
  static const routeName = '/DSaleBillOtherChargeScreen';
  final DSaleBillOtherChargeScreenArguments arguments;

  DSaleBillOtherChargeScreen(this.arguments);
  @override
  BaseState<DSaleBillOtherChargeScreen> createState() =>
      _DSaleBillOtherChargeScreenState();
}

class _DSaleBillOtherChargeScreenState
    extends BaseState<DSaleBillOtherChargeScreen>
  ...
  void getProductFromDBResponse(GetDealerSaleBillProductState state) {
    _inquiryProductList.clear();

    for (int i = 0; i < state.dealerSaleBillProductDBTable.length; i++) {
      _inquiryProductList.add(state.dealerSaleBillProductDBTable[i]);
    }
  }
