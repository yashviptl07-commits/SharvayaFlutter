...
class AddDSalesBillProductAddEdit_screenArguments {
  DealerSaleBillProductDBTable model;
  int StateCode;
  String HeaderDiscAmnt;

  AddDSalesBillProductAddEdit_screenArguments(
      this.model, this.StateCode, this.HeaderDiscAmnt);
}

class DSaleBillProductAddEditScreen extends BaseStatefulWidget {
  static const routeName = '/DSaleBillProductAddEditScreen';
  final AddDSalesBillProductAddEdit_screenArguments arguments;

  DSaleBillProductAddEditScreen(this.arguments);
  @override
  BaseState<DSaleBillProductAddEditScreen> createState() =>
      _DSaleBillProductAddEditScreenState();
}

class _DSaleBillProductAddEditScreenState
    extends BaseState<DSaleBillProductAddEditScreen>
  ...
  List<DealerSaleBillProductDBTable> _inquiryProductList = [];
  List<DealerSaleBillProductDBTable> _TempinquiryProductList = [];
  ...
  List<DealerSaleBillProductDBTable> temp = [];
    ...
    return BlocProvider(
      ...
      child: BlocConsumer<DealerBloc, DealerStates>(
        ...
        listener: (BuildContext context, DealerStates state) {
          if (state is InsertDealerSaleBillProductState) {
            insertResponseFromDB(state);
          }
          if (state is UpdateDealerSaleBillProductState) {
            UpdateResponseFromDB(state);
          }
          return super.build(context);
        },
        listenWhen: (oldState, currentState) {
          if (currentState is InsertDealerSaleBillProductState ||
              currentState is UpdateDealerSaleBillProductState) {
            return true;
          }
          return false;
        },
  ...
  _onTapOfAdd() async {
    ...
    if (isProductExist == false) {
      if (isForUpdate) {
        _dealerBloc.add(UpdateDealerSaleBillProductDBTableCallEvent(
        ...
        _dealerBloc.add(InsertDealerSaleBillProductDBTableCallEvent(
            DealerSaleBillProductDBTable(
                "",
                "",
                productID,
  ...
  void getProductFromDB(GetDealerSaleBillProductState state) {
    ...
    for (int i = 0; i < state.dealerSaleBillProductDBTable.length; i++) {
      temp.add(state.dealerSaleBillProductDBTable[i]);
    }
  }

  void insertResponseFromDB(InsertDealerSaleBillProductState state) {
    showCommonDialogWithSingleOption(context, state.response,
        positiveButtonTitle: "OK");
  }

  void UpdateResponseFromDB(UpdateDealerSaleBillProductState state) {
    showCommonDialogWithSingleOption(context, state.response,
        positiveButtonTitle: "OK");
  }
}
