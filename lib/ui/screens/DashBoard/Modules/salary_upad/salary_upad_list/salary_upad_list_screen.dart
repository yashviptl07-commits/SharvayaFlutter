...
import 'package:soleoserp/blocs/other/bloc_modules/salary_upad/salary_upad_bloc.dart';
...
class SalaryUpadListScreen extends BaseStatefulWidget {
  static const routeName = '/SalaryUpadListScreen';

  @override
  _SalaryUpadListScreenState createState() => _SalaryUpadListScreenState();
}
...
class _SalaryUpadListScreenState extends BaseState<SalaryUpadListScreen>
    with BasicScreen, WidgetsBindingObserver {
  SalaryUpadScreenBloc _CustomerBloc;
  int _pageNo = 0;
  LoanListResponse _inquiryListResponse;
  bool expanded = true;
...
*/
    ...
    _CustomerBloc = SalaryUpadScreenBloc(baseBloc);
    isExpand = false;
    getContacts();
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (BuildContext context) => _CustomerBloc
        ..add(SalaryUpadListCallEvent(
            1,
            SalaryUpadListRequest(
                pkID: "",
                CompanyId: CompanyID.toString(),
                LoginUserID: LoginUserID))),
      child: BlocConsumer<SalaryUpadScreenBloc, SalaryUpadScreenStates>(
        builder: (BuildContext context, SalaryUpadScreenStates state) {
          if (state is SalaryUpadListResponseState) {
        ...
        listener: (BuildContext context, SalaryUpadScreenStates state) {
          /*  if (state is LoanDeleteResponseState) {
            _onCustomerDeleteCallSucess(state, context);
          }*/
          if (state is SalaryUpadDeleteResponseState) {
            _onCustomerDeleteCallSucess(state, context);
          }
          return super.build(context);
        },
        listenWhen: (oldState, currentState) {
          if (currentState is SalaryUpadDeleteResponseState) {
            return true;
          }
          return false;
        },
            ...
            children: [
              Expanded(
                child: RefreshIndicator(
                  onRefresh: () async {
                    _CustomerBloc.add(SalaryUpadListCallEvent(
                        1,
                        SalaryUpadListRequest(
                            pkID: "",
                            CompanyId: CompanyID.toString(),
  ...
  void _onInquiryListCallSuccess(SalaryUpadListResponseState state) {
    if (_pageNo != state.newPage || state.newPage == 1) {
      //checking if new data is arrived
      if (state.newPage == 1) {
        //resetting search
        _searchDetails = null;
        _inquiryListResponse = state.loanListResponse;
      } else {
        _inquiryListResponse.details.addAll(state.loanListResponse.details);
      }
  ...
  ExpantionCustomer(BuildContext context, int index) {
    LoanDetails model = _inquiryListResponse.details[index];
                                ...
                                Expanded(
                                    ...
                                    child: Column(
                                      ...
                                      children: <Widget>[
                                        ...
                                        Text(
                                            model.noOfInstallments == 0
                                                ? "N/A"
                                                : model.noOfInstallments
                                                    .toString(),
                                            style: TextStyle(
  ...
  void _onTapOfDeleteInquiry(int id) {
    ...
    showCommonDialogWithTwoOptions(
      ...
      _CustomerBloc.add(SalaryUpadDeleteCallEvent(
          id, BankVoucherDeleteRequest(CompanyID: CompanyID.toString())));

      //_collapse();
      /*_CustomerBloc.add(LoanDeleteCallEvent(
          id, BankVoucherDeleteRequest(CompanyID: CompanyID.toString())));*/
  ...
  Future<void> _onTapOfSearchView() async {
    navigateTo(context, SearchSalaryUpadScreen.routeName).then((value) {
      if (value != null) {
        ...
        /*_CustomerBloc.add(BankVoucherSearchByIDCallEvent(value.value,
            BankVoucherSearchByIDRequest(
                CompanyId: CompanyID.toString(),
                LoginUserID: LoginUserID)));*/
        _CustomerBloc.add(SalaryUpadSearchCallEvent(LoanSearchRequest(
            pkID: "",
            LoginUserID: LoginUserID,
  ...
  void _onCustomerDeleteCallSucess(
      SalaryUpadDeleteResponseState state, BuildContext context) {
    ...
    if (state.bankVoucherDeleteResponse.details[0].column1.toString() ==
      ...
      navigateTo(context, SalaryUpadListScreen.routeName, clearAllStack: true);
    }
  }
