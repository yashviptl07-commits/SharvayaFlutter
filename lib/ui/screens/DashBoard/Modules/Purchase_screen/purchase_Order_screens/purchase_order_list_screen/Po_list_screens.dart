...
class PoListScreen extends BaseStatefulWidget {
  static const routeName = '/PoListScreen';

  @override
  _PoListScreenState createState() => _PoListScreenState();
}
...
class _PoListScreenState extends BaseState<PoListScreen>
    with BasicScreen, WidgetsBindingObserver {
  MainBloc _mainBloc;
  int _pageNo = 0;
  PurchaseOrderListResponse _purchaseOrderListResponse;
  bool expanded = true;
  ...
  void initState() {
    ...
    _mainBloc.add(PurchaseOrderListEvent(
        1,
    ...
    return BlocProvider(
      ...
      child: BlocConsumer<MainBloc, MainStates>(
        builder: (BuildContext context, MainStates state) {
          if (state is PurchaseOrderListResponseState) {
            _onPurchaseOrderListResponseState(state);
          }
        ...
        buildWhen: (oldState, currentState) {
          if (currentState is PurchaseOrderListResponseState ||
        ...
        listener: (BuildContext context, MainStates state) {
          ...
          if (state is PurchaseOrderDeleteCallState) {
            _onPurchaseOrderDeleteCallState(state);
          }
        },
        listenWhen: (oldState, currentState) {
          if (currentState is SalesBillPDFGenerateResponseState ||
              currentState is PurchaseOrderDeleteCallState) {
            return true;
          }
      ...
      child: Scaffold(
        ...
        floatingActionButton: IsAddRights == true
            ? FloatingActionButton(
                onPressed: () async {
                  await _onTapOfDeleteALLProduct();
                  navigateTo(context, POAddEditScreen.routeName,
                      clearAllStack: true);
                },
  ...
  Widget _buildInquiryList() {
    if (_purchaseOrderListResponse == null) {
      return Container();
    }
        ...
        children: [
          Expanded(
            child: ListView.builder(
              ...
              shrinkWrap: true,
              itemCount: _purchaseOrderListResponse.details.length,
            ),
          ),
        ],
      ),
  ...
  void _onPurchaseOrderListResponseState(PurchaseOrderListResponseState state) {
    if (_pageNo != state.newPage || state.newPage == 1) {
      if (state.newPage == 1) {
        _purchaseOrderListResponse = state.response;
      } else {
        _purchaseOrderListResponse.details.addAll(state.response.details);
      }
...
// Helper styles for labels and titles
  ...
  ExpantionCustomer(BuildContext context, int index) {
    ...
    PurchaseOrderListResponseDetails model =
        _purchaseOrderListResponse.details[index];
              ...
              Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildDetailRow(
                        "Invoice No : ",
                        model.orderNo == null ? "N/A" : model.orderNo,
                        fontSizeLabel,
                        fontSizeLabel * 1.1),
                    _buildDetailRow(
                        "Purchase Exe : ",
                        model.employeeName == null ? "N/A" : model.employeeName,
                        fontSizeLabel,
                        fontSizeLabel * 1.1),
                  ],
                ),
                        ...
                        showCommonDialogWithTwoOptions(
                            ...
                            onTapOfNegativeButton: () async {
                              ...
                              String printHeaderYes = SiteURL +
                                  "/PurchaseOrders.aspx?PrintHeader=yes&MobilePdf=yes&userid=" +
                                  LoginUserID +
                                  "&password=" +
                                  Password +
                                  "&pQuotID=" +
                      ...
                      ? IconButton(
                          onPressed: () {
                            showCommonDialogWithTwoOptions(context,
                              ...
                              Navigator.of(context).pop();
                              _mainBloc.add(PurchaseOrderDeleteCallEvent(
                                PoHeaderDeleteRequest(
  ...
  Future<void> _showMyDialog(
      PurchaseOrderListResponseDetails model, String printWebURL) async {
  ...
  GenerateQT(PurchaseOrderListResponseDetails model, BuildContext context123,
      String printWebURL) {
    return Center(
      child: Stack(
        children: [
                    ...
                    setState(() {
                      ...
                      if (pageTitle == "E-Office-Desk") {
                        ...
                        showCommonDialogWithSingleOption(
                            context, "Purchase Order Generated Successfully ",
                            onTapOfPositiveButton: () {
                          Navigator.of(context).pop();
  ...
  void getUserRights(MenuRightsResponse menuRightsResponse) {
    for (int i = 0; i < menuRightsResponse.details.length; i++) {
      if (menuRightsResponse.details[i].menuName == "pgPurcOrder") {
  ...
  void _onPurchaseOrderDeleteCallState(PurchaseOrderDeleteCallState state) {
  ...
  void _onTapOfEditData(PurchaseOrderListResponseDetails model) async {
    navigateTo(context, POAddEditScreen.routeName,
            arguments: POAddEditScreenArguments(model))
        .then((value) {
      _mainBloc.add(PurchaseOrderListEvent(
          1,
          PurchaseOrderListRequest(
