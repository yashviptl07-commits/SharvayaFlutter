...
class POProductAddEditScreenArguments {
  PurchaseOrderTable model;
  int StateCode;
  String HeaderDiscAmnt;
  String QuotationNo;

  POProductAddEditScreenArguments(
      this.model, this.StateCode, this.HeaderDiscAmnt, this.QuotationNo);
}

class POProductAddEditScreen extends BaseStatefulWidget {
  static const routeName = '/POProductAddEditScreen';
  final POProductAddEditScreenArguments arguments;

  POProductAddEditScreen(this.arguments);

  @override
  _POProductAddEditScreenState createState() => _POProductAddEditScreenState();
}

class _POProductAddEditScreenState extends BaseState<POProductAddEditScreen>
  ...
  Widget buildBody(BuildContext context) {
    return Column(
      children: [
        getCommonAppBar(
            context, baseTheme, "${isForUpdate ? "Update" : "Add"} Po Product",
  ...
  _onTapOfAdd() async {
    ...
    if (isProductExist == false) {
      if (isForUpdate) {
        await OfflineDbHelper.getInstance().updatePurchaseOrderProduct(
            PurchaseOrderTable(
                "",
                Specification,
                productID,
  ...
  Future<void> _onTapOfSearchView() async {
    ...
    _mainBloc.add(ConstantRequestEvent(
        CompanyID.toString(),
        ConstantRequest(
