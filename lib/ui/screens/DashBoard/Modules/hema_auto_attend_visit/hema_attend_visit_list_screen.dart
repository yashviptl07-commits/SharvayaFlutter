...
class HemaAttendVisitListScreen extends BaseStatefulWidget {
  ...
  @override
  _HemaAttendVisitListScreenState createState() =>
...
class _HemaAttendVisitListScreenState
          ...
          actions: <Widget>[
            ...
            IsAddRights == true
                ? IconButton(
                    ...
                    onPressed: () {
                      navigateTo(
                          context, HemaAttendVisitAddEditScreen.routeName,
                          clearAllStack: true);
                    })
                : Container(),
            SizedBox(width: 10)
  ...
  Widget _buildInquiryList() {
    ...
    double smallFontSize = deviceWidth * 0.025;
                    ...
                    children: [
                      GestureDetector(
                        onTap: () async {
                          if (model.latitudeIN != "" ||
                              model.longitudeIN != "") {
                            double value1 = double.parse(model.latitudeIN);
                            double value2 = double.parse(model.longitudeIN);

                            MapsLauncher.launchCoordinates(
                                value1, value2, 'Location In');
                          } else {
                      ...
                      GestureDetector(
                        onTap: () async {
                          if (model.latitudeOUT != "" ||
                              model.longitudeOUT != "") {
                            double value1 = double.parse(model.latitudeOUT);
                            double value2 = double.parse(model.longitudeOUT);
                            MapsLauncher.launchCoordinates(
                                value1, value2, 'Location Out');
                          } else {
                            showCommonDialogWithSingleOption(
                                context, "Location Out Not Valid !",
                                positiveButtonTitle: "OK",
                                onTapOfPositiveButton: () {
                              Navigator.of(context).pop();
                            });
  ...
  void _ongetDocumentListResponse(DefDocumentListResponseState state) async {
    documentList.clear();
    ...
    navigateTo(context, HemaAttendVisitAddEditScreen.routeName,
            arguments: QuickAddUpdateComplaintScreenArguments(
                state.vehicleDefDetails, documentList))
        .then((value) {
      _complaintScreenBloc.add(AttendVisitListCallEvent(
          1,
          AttendVisitListRequest(
  ...
  void _onTapOfEditCustomer(AttendVisitDetails model) {
    _complaintScreenBloc.add(DefDocumentListRequestEvent(
