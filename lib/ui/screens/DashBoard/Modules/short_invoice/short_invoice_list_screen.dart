...
class ShortInvoiceListScreen extends BaseStatefulWidget {
  static const routeName = '/ShortInvoiceListScreen';

  @override
  _ShortInvoiceListScreenState createState() => _ShortInvoiceListScreenState();
}
...
class _ShortInvoiceListScreenState extends BaseState<ShortInvoiceListScreen>
    with BasicScreen, WidgetsBindingObserver {
  MainBloc _mainBloc;
  int _pageNo = 0;
  ShortInvoiceListResponse _shortInvoiceListResponse;
  bool expanded = true;
  ...
  void initState() {
    ...
    _mainBloc.add(PaymentScheduleDeleteAllItemEvent());
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (BuildContext context) => _mainBloc
        ..add(ShortInvoiceListRequestEvent(
            1,
            ShortInvoiceListRequest(
                LoginUserID: LoginUserID,
                SearchKey: edt_CustomerName.text,
      ...
      child: BlocConsumer<MainBloc, MainStates>(
        builder: (BuildContext context, MainStates state) {
          if (state is ShortInvoiceListResponseState) {
        ...
        buildWhen: (oldState, currentState) {
          if (currentState is ShortInvoiceListResponseState ||
      ...
      child: Scaffold(
        ...
        floatingActionButton: IsAddRights == true
            ? FloatingActionButton(
                onPressed: () async {
                  await _onTapOfDeleteALLProduct();
                  navigateTo(context, ShortInvoiceNewAddEditScreen.routeName);
                },
  ...
  Widget _buildInquiryList() {
    if (_shortInvoiceListResponse == null) {
      return Container();
    }
        ...
        children: [
          Expanded(
            child: ListView.builder(
              ...
              shrinkWrap: true,
              itemCount: _shortInvoiceListResponse.details.length,
            ),
          ),
        ],
      ),
  ...
  void _onInquiryListCallSuccess(ShortInvoiceListResponseState state) {
    if (_pageNo != state.newPage || state.newPage == 1) {
      if (state.newPage == 1) {
        _shortInvoiceListResponse = state.response;
      } else {
        _shortInvoiceListResponse.details.addAll(state.response.details);
      }
...
// Helper styles for labels and titles
  ...
  ExpantionCustomer(BuildContext context, int index) {
    ...
    ShortInvoiceListResponseDetails model =
        _shortInvoiceListResponse.details[index];
                        ...
                        showCommonDialogWithTwoOptions(
                            ...
                            onTapOfNegativeButton: () async {
                              ...
                              String printHeaderYes = SiteURL +
                                  "/shortinvoice.aspx?PrintHeader=yes&MobilePdf=yes&userid=" +
                                  LoginUserID +
                                  "&password=" +
                                  Password +
                                  "&pQuotID=" +
                      ...
                      ? IconButton(
                          onPressed: () {
                            showCommonDialogWithTwoOptions(context,
                              ...
                              Navigator.of(context).pop();
                              _mainBloc.add(ShortInvoiceDeleteRequestEvent(
  ...
  Future<void> _showMyDialog(
      ShortInvoiceListResponseDetails model, String printWebURL) async {
  ...
  GenerateQT(ShortInvoiceListResponseDetails model, BuildContext context123,
      String printWebURL) {
    return Center(
      child: Stack(
        children: [
                    ...
                    setState(() {
                      ...
                      if (pageTitle == "E-Office-Desk") {
                        ...
                        showCommonDialogWithSingleOption(
                            context, "Short Invoice Generated Successfully ",
                            onTapOfPositiveButton: () {
                          Navigator.of(context).pop();
  ...
  void getUserRights(MenuRightsResponse menuRightsResponse) {
    for (int i = 0; i < menuRightsResponse.details.length; i++) {
      ...
      if (menuRightsResponse.details[i].menuName == "pgShortInvoice") {
  ...
  void _onShortInvoiceDeleteResponseState(
      ShortInvoiceDeleteResponseState state) {
  ...
  void _onTapOfEditData(ShortInvoiceListResponseDetails model) async {
    navigateTo(context, ShortInvoiceNewAddEditScreen.routeName,
            arguments: ShortInvoiceNewAddEditScreenArguments(model))
        .then((value) {
      _mainBloc.add(ShortInvoiceListRequestEvent(
          1,
          ShortInvoiceListRequest(
