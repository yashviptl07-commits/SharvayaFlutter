...
class ShortInvoiceAddEditScreenArguments {
  ShortInvoiceTable model;
  int StateCode;
  String HeaderDiscAmnt;
  String QuotationNo;

  ShortInvoiceAddEditScreenArguments(
      this.model, this.StateCode, this.HeaderDiscAmnt, this.QuotationNo);
}

class ShortInvoiceAddEditScreen extends BaseStatefulWidget {
  static const routeName = '/ShortInvoiceAddEditScreen';
  final ShortInvoiceAddEditScreenArguments arguments;

  ShortInvoiceAddEditScreen(this.arguments);

  @override
  _ShortInvoiceAddEditScreenState createState() =>
      _ShortInvoiceAddEditScreenState();
}

class _ShortInvoiceAddEditScreenState
    extends BaseState<ShortInvoiceAddEditScreen>
  ...
  void initState() {
    ...
    if (widget.arguments.model != null) {
      ...
      _quantityController.text = widget.arguments.model.Qty.toString();
      _unitController.text = widget.arguments.model.Unit.toString();
      _unitPriceController.text =
          widget.arguments.model.Rate.toStringAsFixed(2);
      _netRateController.text =
          widget.arguments.model.NetRate.toStringAsFixed(2);
      _discPerController.text =
          widget.arguments.model.DiscountPer.toStringAsFixed(2);
      ...
      _taxPerController.text =
          widget.arguments.model.AddTaxPer.toStringAsFixed(2);
      _taxAmountController.text =
          widget.arguments.model.AddTaxAmt.toStringAsFixed(2);
      _totalAmountController.text =
          widget.arguments.model.NetAmt.toStringAsFixed(2);
    ...
    return Column(
      children: [
        getCommonAppBar(context, baseTheme,
            "${isForUpdate ? "Update" : "Add"} Short Invoice Product",
    ...
    if (isProductExist == false) {
      if (isForUpdate) {
        await OfflineDbHelper.getInstance()
            .updateShortInvoice(ShortInvoiceTable(
                0, //int pkID,
                "", //String InvoiceNo,
                "", //String DocRefNo,
                ...
                0, //int LocationID,
                ISTaxType, //int TaxType,
                0.00, //double PendingQty,
                0.00, //double UnitQty,
                quantity, //double Qty,
                unit, //String Unit,
                unitRate, //double Rate,
                disc, //double DiscountPer,
                discAmount, //double DiscountAmt,
                ...
                taxPer, //double AddTaxPer,
                taxAmount, //double AddTaxAmt,
                netAmount, //double NetAmt,
  ...
  TotalAmountCalculation1() async {
    ...
    List<ShortInvoiceTable> temp2 = [];
    if (temp.length != 0) {
      for (int i = 0; i < temp.length; i++) {
        if (temp[i].TaxType == 1) {
          Exclusivetot_amount = temp[i].Qty * temp[i].NetRate;
          Exclusivetot_tax_amt =
              (Exclusivetot_amount * temp[i].AddTaxPer) / 100;
          Exclusivetot_amnt_net = Exclusivetot_amount + Exclusivetot_tax_amt;
          ExTotalNetAmnt += Exclusivetot_amnt_net;
        } else {
          Inclusivetot_amount = temp[i].Qty * temp[i].NetRate;
          Inclusivetot_tax_amt =
              ((temp[i].Qty * temp[i].NetRate) * temp[i].AddTaxPer) /
                  (100 + temp[i].AddTaxPer);
          Inclusivetot_amnt_net = Inclusivetot_amount;
    ...
    if (_quantityController.text.toString() != null &&
      ...
      setState(() {
        ...
        var TaxPer = _taxPerController.text == ""
            ? 0.00
            : double.parse(_taxPerController.text.toString());
        var Amount1 = 0.00;
        var TaxAmount1 = 0.00;
  ...
  Future<void> getInquiryDetailsFromDb() async {
    List<ShortInvoiceTable> temp =
        await OfflineDbHelper.getInstance().getShortInvoiceList();
    for (int i = 0; i < temp.length; i++) {
      TotalNetAmnt = TotalNetAmnt + temp[i].NetAmt;
    }
  }
