...
class NewShortInvoiceOtherChargesScreenArguments {
  int StateCode;
  ...
  NewShortInvoiceOtherChargesScreenArguments(
      this.StateCode,
      this.editModel,
...
class NewShortInvoiceOtherChargeScreen extends BaseStatefulWidget {
  static const routeName = '/NewShortInvoiceOtherChargeScreen';
  final NewShortInvoiceOtherChargesScreenArguments arguments;
  NewShortInvoiceOtherChargeScreen(this.arguments);

  @override
  _NewShortInvoiceOtherChargeScreenState createState() =>
      _NewShortInvoiceOtherChargeScreenState();
}

class _NewShortInvoiceOtherChargeScreenState
    extends BaseState<NewShortInvoiceOtherChargeScreen>
  ...
  TextEditingController _AdvanceAmt = TextEditingController();
  TextEditingController _AdvancePer = TextEditingController();
  ...
  List<ShortInvoiceTable> productList = [];
  ...
  void initState() {
    ...
    _mainBloc.add(QuotationOtherCharge1CallEvent(
        CompanyID.toString(), QuotationOtherChargesListRequest(pkID: "")));

    _mainBloc.add(GetGenericAdditionalChargesEvent());

    _mainBloc.add(QuotationOtherChargeCallEvent(_headerDiscountController.text,
        CompanyID.toString(), QuotationOtherChargesListRequest(pkID: "")));
    ...
    return BlocProvider(
      ...
      child: BlocConsumer<MainBloc, MainStates>(
        builder: (BuildContext context, MainStates state) {
          ...
          if (state is GetGenericAdditionalChargesState) {
        ...
        buildWhen: (oldState, currentState) {
          if (currentState is GetSIProductListState ||
              ...
              currentState is GetGenericAdditionalChargesState) {
            return true;
          }
          return false;
        },
        listener: (BuildContext context, MainStates state) {
          if (state is InsertProductSuccessResponseState) {
        ...
        listenWhen: (oldState, currentState) {
          if (currentState is InsertProductSuccessResponseState ||
  ...
  Future<bool> _onBackPressed() async {
    AddditionalCharges addditionalCharges = AddditionalCharges(
      ...
      AdvanceAmt: _AdvanceAmt.text.toString(),
      AdvancePer: _AdvancePer.text.toString(),
    );
  ...
  void _OnGetQuotationProductList(GetSIProductListState state) {
  ...
  void _OnTaptoSave() {
    ...
    if (productList != null) {
      ...
      _mainBloc.add(InsertProductEvent(TempproductList));
      ...
      _mainBloc
          .add(AddGenericAdditionalChargesEvent(newGenericAddditionalCharges));
    }
  }

  void UpdateHeaderDiscountCalculation(
      List<ShortInvoiceTable> tempproductList) {
...
*/
  ...
  void _OnProductSucessResponse(InsertProductSuccessResponseState state) {
  ...
  void _OngetAdditionalCharges(GetGenericAdditionalChargesState state) {
  ...
  void UpdatefromaddeditScreen(List<ShortInvoiceTable> tempproductList) {
    if (tempproductList != null) {
      double Tot_BasicAmount = 0.00;
      double Tot_GSTAmt = 0.00;
      double Tot_CGSTAmt = 0.00;
      ...
      _AdvanceAmt.text = _addditionalCharges.AdvanceAmt.toString() == "null"
          ? "0.00"
          : _addditionalCharges.AdvanceAmt.toString();
      _AdvancePer.text = _addditionalCharges.AdvancePer.toString() == "null"
          ? "0.00"
          : _addditionalCharges.AdvancePer.toString();
        ...
        Card(
          ...
          child: Container(
            ...
            child: TextFormField(
                ...
                textInputAction: TextInputAction.next,
                keyboardType: TextInputType.numberWithOptions(decimal: true),
                controller: _AdvancePer,
                onTap: () => {
                      _AdvancePer.selection = TextSelection(
                        baseOffset: 0,
                        extentOffset: _AdvancePer.text.length,
                      )
                    },
                onChanged: (value) {
                  double netamnt = _netAmountController.text == ""
                      ? 0.00
                      : double.parse(_netAmountController.text);
                  double advper = value == "" ? 0.00 : double.parse(value);

                  double tot_amt = netamnt * advper;
                  double tot_amt1 = tot_amt / 100;

                  _AdvanceAmt.text = tot_amt1.toStringAsFixed(2);
                },
                decoration: InputDecoration(
                  contentPadding: EdgeInsets.only(bottom: 10),
                  hintText: "0.00",
