...
class ShortInvoiceNewAddEditScreenArguments {
  ShortInvoiceListResponseDetails editModel;

  ShortInvoiceNewAddEditScreenArguments(this.editModel);
}

class ShortInvoiceNewAddEditScreen extends BaseStatefulWidget {
  static const routeName = '/ShortInvoiceNewAddEditScreen';

  final ShortInvoiceNewAddEditScreenArguments arguments;

  ShortInvoiceNewAddEditScreen(this.arguments);

  @override
  _ShortInvoiceNewAddEditScreenState createState() =>
      _ShortInvoiceNewAddEditScreenState();
}

class _ShortInvoiceNewAddEditScreenState
    extends BaseState<ShortInvoiceNewAddEditScreen>
  ...
  List<ALL_Name_ID> arr_ALL_Name_ID_For_RefSelectNo = [];
  ...
  ShortInvoiceListResponseDetails _editModel;
  ...
  List<ShortInvoiceTable> _inquiryProductList = [];
  List<ShortInvoiceProductRequest> arrSOProductList = [];
    ...
    return BlocProvider(
      ...
      child: BlocConsumer<MainBloc, MainStates>(
        builder: (BuildContext context, MainStates state) {
          ...
          if (state is ShortInvoiceShipmentListResponseState) {
            _OnShortInvoiceShipmentListResponseState(state);
          }
          if (state is ShortInvoiceExportListResponseState) {
            _OnShortInvoiceExportListResponseState(state);
          }
          if (state is ShortInvoiceDetailsListResponseState) {
        ...
        buildWhen: (oldState, currentState) {
          if (currentState is BankDetailsListResponseState ||
              ...
              currentState is ShortInvoiceShipmentListResponseState ||
              currentState is ShortInvoiceExportListResponseState ||
              currentState is ShortInvoiceDetailsListResponseState ||
        ...
        listener: (BuildContext context, MainStates state) {
          ...
          if (state is ShortInvoiceAddUpdateResponseState) {
            _onShortInvoiceAddUpdateResponseState(state);
          }
          if (state is ShortInvoiceProductSaveResponseState) {
          ...
          if (state is ShortInvoiceExportAddUpdateResponseState) {
            _onShortInvoiceExportAddUpdateResponseState(state);
          }
          if (state is ShortInvoiceShipmentAddUpdateResponseState) {
            _onShortInvoiceShipmentAddUpdateResponseState(state);
          }
          if (state is ShortInvoiceDetailsDeleteResponseState) {
        ...
        listenWhen: (oldState, currentState) {
          if (currentState is ProductMainListResponseState ||
              ...
              currentState is ShortInvoiceAddUpdateResponseState ||
              currentState is ShortInvoiceProductSaveResponseState ||
              currentState is ShortInvoiceExportAddUpdateResponseState ||
              currentState is ShortInvoiceShipmentAddUpdateResponseState ||
              currentState is ShortInvoiceAssemblyLoadListResponseState ||
              currentState is ShortInvoiceDetailsDeleteResponseState) {
            return true;
          }
          return false;
        },
      ...
      child: DefaultTabController(
        ...
        child: Scaffold(
            appBar: NewGradientAppBar(
              ...
              title: Text("Short Invoice ${_isForUpdate ? "Update" : "Add"}"),
            ),
            body: isLoading == true
  ...
  Future<bool> _onBackPressed() async {
    await _onTapOfDeleteALLProduct();
    navigateTo(context, ShortInvoiceListScreen.routeName, clearAllStack: true);
  }
          ...
          child: Column(
            children: [
                  ...
                  ? Column(
                      children: [
                        ...
                        CustomDropDownForRef(
                          ...
                          enable1: false,
                          icon: Icon(Icons.arrow_drop_down),
                          controllerVehical: edt_RefSelectedNo,
                          vehicalList: arr_ALL_Name_ID_For_RefSelectNo,
                        ),
              ...
              CustomDropDownProducts(
                ...
                enable1: false,
                icon: Icon(Icons.arrow_drop_down),
                controllerVehical: edt_ProductName,
                vehicalList: arr_ALL_Name_ID_For_Product,
              ),
              SizedBox(
                height: 10,
              ),
                  ...
                  onPressed: () async {
                    if (edt_customerName.text != "") {
                      _mainBloc.add(ShortInvoiceAssemblyLoadListRequestEvent(
                          int.parse(edt_ProductID.text),
                          ShortInvoiceAssemblyLoadRequest(
                              FinishProductID: edt_ProductID.text,
                              CompanyId: CompanyID.toString())));
                    } else {
                      showCommonDialogWithSingleOption(
  ...
  void _OnINQ_QT_SO_NO_Response(
    ...
    arr_ALL_Name_ID_For_RefSelectNo.clear();
    if (state.response.details.length != 0) {
      for (var i = 0; i < state.response.details.length; i++) {
        ...
        arr_ALL_Name_ID_For_RefSelectNo.add(categoryResponse123);
      }
    }

    showcustomdialogWithOnlyName(
        values: arr_ALL_Name_ID_For_RefSelectNo,
        context1: context,
        controller: edt_RefSelectedNo,
        lable: "Select No ");
  }
          ...
          children: [
            Container(
              ...
              child: Column(
                children: [
                  _buildSingleField(
                    "Pre Carriage By (Transporter Name)",
                    createTextFormField(
                      _controller_transport_name,
                  ...
                  const SizedBox(height: 8),
                  _buildSingleField(
                    "Place Of Rec.By Pre Carrier",
                    createTextFormField(
                      _controller_place_of_rec,
                      "Enter Place",
                      keyboardInput: TextInputType.text,
                    ),
                  ),
                  const SizedBox(height: 8),
                  _buildRowWithTwoFields(
                    leftLabel: "Vessel/Flight No",
                    rightLabel: "Port Of Loading",
                    leftWidget: createTextFormField(
                      _controller_flight_no,
                      "Enter Flight No.",
                    ),
                    rightWidget: createTextFormField(
                  ...
                  const SizedBox(height: 8),
                  _buildRowWithTwoFields(
                    leftLabel: "Port Of Dispatch",
                    rightLabel: "Port Of Destination",
                    leftWidget: createTextFormField(
                  ...
                  const SizedBox(height: 8),
                  _buildRowWithTwoFields(
                    leftLabel: "Packages Types",
                    rightLabel: "Net Weight(KGs)",
                    leftWidget: createTextFormField(
                    ...
                    rightWidget: createTextFormField(
                      _controller_net_weight,
                      "Enter Net Weight",
                    ),
                  ),
                  const SizedBox(height: 8),
                  _buildRowWithTwoFields(
                    leftLabel: "Gross Weight(KGs)",
                    rightLabel: "FOB (Free Of Board)",
                    leftWidget: createTextFormField(
                    ...
                    if (edt_bankName.text != "") {
                      ...
                      if (temp.length != 0) {
                        showCommonDialogWithTwoOptions(context,
                          ...
                          if (SalesOrderNo != '') {
                            _mainBloc.add(ShortInvoiceDetailsDeleteCallEvent(
                          ...
                          List<ShortInvoiceTable> TempproductList1 =
                              ShortInvoiceHeaderDiscountCalculation
                          ...
                          _mainBloc.add(ShortInvoiceAddUpdateRequestEvent(
                            context,
                            ShortInvoiceAddUpdateRequest(
                              ...
                              ComplaintNo: "",
                              RefType: "",
                              SupplierRef: edt_SuppRef.text,
                              SupplierRefDate: edt_rev_ref_date.text,
                              EmailSubject: edt_select_emailSubjectName.text,
                              ...
                              CRDays: edt_crDays.text,
                              DueDate: _controller_LR_date_Reveres.text,
                              NetAmt: finalPrice[17].toStringAsFixed(2),
                              ModeOfTransport:
                                  _controller_mode_of_transfer.text,
                              TransporterName: _controller_transport_name.text,
                              DeliverTo: _controller_DeliverTo.text,
                              VehicleNo: _controller_vihical_no.text,
                              LRNo: _controller_LR_NO.text,
                              DeliveryNote: _controller_Delivery_Notes.text,
                              DeliveryDate: edt_rev_deliveryDate.text,
                              DispatchDocNo: "",
                              LRDate: _controller_LR_date_Reveres.text,
                              EwayBillNo: _controller_e_way_bill_No.text,
                              ModeOfPayment: _controller_Mode_of_Payment.text,
                              ...
                              CurrencySymbol: "",
                              ExchangeRate: "0.00",
                              ChallanDate: _controller_LR_date_Reveres.text,
                              ChallanNo: "",
                              LoginUserID: LoginUserID,
                              CompanyId: CompanyID.toString(),
                            ),
  ...
  void fillData() async {
    ...
    try {
      ...
      edt_serialNo.text = _editModel.marksNo ?? '';
      edt_dispatchDate.text = _editModel.docRefNoListDate?.getFormattedDate(
              fromFormat: "yyyy-MM-ddTHH:mm:ss", toFormat: "dd-MM-yyyy") ??
          '';
      edt_rev_dispatchDate.text = _editModel.docRefNoListDate?.getFormattedDate(
              fromFormat: "yyyy-MM-ddTHH:mm:ss", toFormat: "yyyy-MM-dd") ??
          '';
      edt_SuppRef.text = _editModel.supplierRef ?? '';
      edt_ref_date.text = _editModel.supplierRefDate?.getFormattedDate(
              fromFormat: "yyyy-MM-ddTHH:mm:ss", toFormat: "dd-MM-yyyy") ??
          '';
      edt_rev_ref_date.text = _editModel.supplierRefDate?.getFormattedDate(
              fromFormat: "yyyy-MM-ddTHH:mm:ss", toFormat: "yyyy-MM-dd") ??
          '';
      edt_projectName.text = _editModel.projectName ?? '';
      edt_crDays.text = _editModel.cRDays?.toString() ?? '';
      edt_deliveryDate.text = _editModel.deliveryDate?.getFormattedDate(
              fromFormat: "yyyy-MM-ddTHH:mm:ss", toFormat: "dd-MM-yyyy") ??
          '';
      edt_rev_deliveryDate.text = _editModel.deliveryDate?.getFormattedDate(
              fromFormat: "yyyy-MM-ddTHH:mm:ss", toFormat: "yyyy-MM-dd") ??
          '';
      ...
      _controller_Delivery_Notes.text = _editModel.deliveryNote ?? '';
      _controller_e_way_bill_No.text = _editModel.ewayBillNo ?? '';
      _controller_Mode_of_Payment.text = _editModel.modeOfPayment ?? '';
      _controller_DeliverTo.text = _editModel.deliverTo ?? '';
      ...
      edt_select_emailSubjectName.text = _editModel.emailSubject ?? '';
      ...
      print("object" + stateCode.toString());
      ...
      if ((_editModel.invoiceNo?.toString() ?? '').isNotEmpty) {
        await _mainBloc.add(ShortInvoiceDetailsListRequestEvent(
            stateCode,
            LoginUserID,
            ShortInvoiceDetailsListRequest(
        ...
        await _mainBloc.add(
          ShortInvoiceExportListRequestEvent(
        ...
        await _mainBloc.add(
          ShortInvoiceShipmentListRequestEvent(
            ShortInvoiceShipmentListRequest(
              pkID: '0',
              InvoiceNo: _editModel.invoiceNo,
              LoginUserID: LoginUserID,
              CompanyId: CompanyID.toString(),
  ...
  void _onShortInvoiceAddUpdateResponseState(
      ShortInvoiceAddUpdateResponseState state) {
    int returnPKID = 0;
    String returnInvoiceNo = "";

    // Extract pkID and InvoiceNo from response
    for (final detail in state.shortInvoiceAddUpdateResponse.details) {
      returnPKID = int.tryParse(detail.column1.toString()) ?? 0;
      returnInvoiceNo = detail.column3 ?? "";
    }

    // Prepare and dispatch export save request
    final exportRequest = ShortInvoiceExportSaveRequest(
      pkID: ShipmentPkID.toString(),
      InvoiceNo: returnInvoiceNo,
      PreCarrBy: _controller_transport_name.text,
      PreCarrRecPlace: _controller_place_of_rec.text,
      FlightNo: _controller_flight_no.text,
      PortOfLoading: _controller_port_of_loading.text,
      PortOfDispatch: _controller_port_of_dispatch.text,
      PortOfDestination: _controller_port_of_destination.text,
      MarksNo: _controller_container_no.text,
      Packages: _controller_packages.text,
      NetWeight: _controller_net_weight.text,
      GrossWeight: _controller_gross_weight.text,
      PackageType: _controller_type_of_package.text,
      FreeOnBoard: _controller_FOB.text,
      LoginUserID: LoginUserID,
      CompanyId: CompanyID.toString(),
    );

    _mainBloc.add(ShortInvoiceExportAddUpdateRequestEvent(exportRequest));

    // Prepare and dispatch shipment save request
    final shipmentRequest = ShortInvoiceShipmentSaveRequest(
      pkID: ShipmentPkID.toString(),
      InvoiceNo: returnInvoiceNo,
    ...
    _mainBloc.add(ShortInvoiceShipmentAddUpdateRequestEvent(shipmentRequest));
  ...
  void updateRetrunInquiryNoToDB(
    ...
    for (int i = 0; i < TempproductList.length; i++) {
      ShortInvoiceProductRequest shortInvoiceProductRequest =
          ShortInvoiceProductRequest(
        pkID: "0",
        InvoiceNo: retrunSO_No,
        DocRefNo: _isForUpdate == true
            ? TempproductList[i].DocRefNo
            : edt_RefSelectedNo.text,
        ...
        UnitQty: TempproductList[i].UnitQty,
        PendingQty: TempproductList[i].PendingQty,
        ...
        ForOrderNo: TempproductList[i].ForOrderNo,
        NetWT: double.parse(TempproductList[i].NetWt),
        GrossWT: double.parse(TempproductList[i].GrossWt),
        BoxNo: TempproductList[i].BocNo,
        LoginUserID: LoginUserID,
        CompanyId: CompanyID,
      );

      arrSOProductList.add(shortInvoiceProductRequest);
    }

    _mainBloc.add(ShortInvoiceProductSaveCallEvent(
        context1, retrunSO_No, arrSOProductList));
  }

  void _OnShortInvoiceProductSaveResponseState(
      ShortInvoiceProductSaveResponseState state) async {
    String Msg = _isForUpdate == true
        ? "ShortInvoice Updated Successfully"
        : "ShortInvoice Added Successfully";
                  ...
                  _buildChargesButton(
                    ...
                    onPressed: () async {
                      ...
                      if (_inquiryProductList.isNotEmpty) {
                        navigateTo(
                          context,
                          NewShortInvoiceOtherChargeScreen.routeName,
                          arguments: NewShortInvoiceOtherChargesScreenArguments(
  ...
  void _OnShortInvoiceShipmentListResponseState(
      ShortInvoiceShipmentListResponseState state) async {
    if (state.shortInvoiceShipmentListResponse.details.isNotEmpty) {
      final detail = state.shortInvoiceShipmentListResponse.details[0];
  ...
  void _OnShortInvoiceExportListResponseState(
      ShortInvoiceExportListResponseState state) async {
    if (state.shortInvoiceExportListResponse.details.isNotEmpty) {
      final detail = state.shortInvoiceExportListResponse.details[0];

      ExportPkID = detail.pkID ?? '0';
      _controller_transport_name.text = detail.preCarrBy ?? '';
      _controller_place_of_rec.text = detail.preCarrRecPlace ?? '';
      _controller_flight_no.text = detail.flightNo ?? '';
      _controller_port_of_loading.text = detail.portOfLoading ?? '';
      _controller_port_of_dispatch.text = detail.portOfDispatch ?? '';
      _controller_port_of_destination.text = detail.portOfDestination ?? '';
      _controller_container_no.text = detail.marksNo ?? '';
      _controller_packages.text = detail.packages ?? '';
      _controller_type_of_package.text = detail.packageType ?? '';
      _controller_net_weight.text = detail.netWeight ?? '';
      _controller_gross_weight.text = detail.grossWeight ?? '';
      _controller_FOB.text = detail.freeOnBoard ?? '';
    }
  }

  void _onShortInvoiceDetailsListResponseState(
      ShortInvoiceDetailsListResponseState state) {}

  void _onShortInvoiceExportAddUpdateResponseState(
      ShortInvoiceExportAddUpdateResponseState state) {
    print(state.response.details[0].column2);
  }

  void _onShortInvoiceShipmentAddUpdateResponseState(
      ShortInvoiceShipmentAddUpdateResponseState state) {
    print(state.response.details[0].column2);
  }

  void _onShortInvoiceDetailsDeleteResponseState(
      ShortInvoiceDetailsDeleteResponseState state) {}
    ...
    if (state.response.details.isNotEmpty) {
      for (var detail in state.response.details) {
        ...
        bool exists = await OfflineDbHelper.getInstance()
            .isProductAlreadyStored(productId);
        if (exists) continue;
