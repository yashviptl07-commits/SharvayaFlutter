...
class MissedPunchApprovalListScreen extends BaseStatefulWidget {
  ...
  @override
  _MissedPunchApprovalListScreenState createState() =>
      _MissedPunchApprovalListScreenState();
}

class _MissedPunchApprovalListScreenState
    extends BaseState<MissedPunchApprovalListScreen>
    with BasicScreen, WidgetsBindingObserver {
  MissedPunchScreenBloc _leaveRequestScreenBloc;
  int _pageNo = 0;
  bool isListExist = false;
  MissedPunchApprovalListResponse _leaveRequestListResponse;
  ...
  void _fetchList() {
    _leaveRequestScreenBloc.add(MissedPunchApprovalListCallEvent(
    ...
    return BlocProvider(
      ...
      child: BlocConsumer<MissedPunchScreenBloc, MissedPunchScreenStates>(
        builder: (BuildContext context, MissedPunchScreenStates state) {
          if (state is MissedPunchApprovalListResponseState) {
            _onMissedPunchApprovalListResponse(state);
          }
          return super.build(context);
        },
        buildWhen: (oldState, currentState) {
          if (currentState is MissedPunchApprovalListResponseState) {
            return true;
          }
          return false;
        },
        listener: (BuildContext context, MissedPunchScreenStates state) {
          if (state is MissedPunchApprovalSaveResponseState) {
            _OnMissedPunchApprovalSaveSucess(state);
          }
        },
        listenWhen: (oldState, currentState) {
          if (currentState is MissedPunchApprovalSaveResponseState) {
            return true;
          }
          return false;
        },
            ...
            children: [
              ...
              Expanded(
                child: RefreshIndicator(
                  ...
                  child: _buildMissedPunchList(context),
                ),
              ),
            ],
          ),
  ...
  Widget _buildMissedPunchList(BuildContext context) {
    final r = _R(context);
    if (isListExist && _leaveRequestListResponse != null) {
      return ListView.builder(
        ...
        itemBuilder: (ctx, i) => _buildMissedPunchCard(ctx, i),
      );
    }
    return Center(
  ...
  Widget _buildMissedPunchCard(BuildContext context, int index) {
    final r = _R(context);
                    ...
                    child: Column(
                      ...
                      children: [
                        _infoTile(
                          ...
                          "Miss.Punch Date",
                          _fmtDate(detail.presenceDate ?? "N/A"),
                        ),
                        SizedBox(height: r.s(7)),
                        _infoTile(
                          r,
  ...
  void _onMissedPunchApprovalListResponse(
      MissedPunchApprovalListResponseState state) {
    _leaveRequestListResponse = state.missedPunchApprovalListResponse;
  ...
  void _onTapOfEditCustomer(MissedPunchApprovalDetails detail) {
    ...
    showCommonDialogWithTwoOptions(
      ...
      onTapOfPositiveButton: () {
        ...
        _leaveRequestScreenBloc.add(MissedPunchApprovalSaveRequestCallEvent(
            detail.pkID,
  ...
  void _OnMissedPunchApprovalSaveSucess(
      MissedPunchApprovalSaveResponseState state) {
    showCommonDialogWithSingleOption(
        context, "Missed Punch Approval Updated Successfully!",
