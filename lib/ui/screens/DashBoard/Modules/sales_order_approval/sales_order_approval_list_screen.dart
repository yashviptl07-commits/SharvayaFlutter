...
class SalesOrderApprovalListScreen extends BaseStatefulWidget {
  static const routeName = '/SalesOrderApprovalListScreen';

  @override
  _SalesOrderApprovalListScreenState createState() =>
      _SalesOrderApprovalListScreenState();
}

class _SalesOrderApprovalListScreenState
    extends BaseState<SalesOrderApprovalListScreen>
    with BasicScreen, WidgetsBindingObserver {
  SalesOrderBloc _salesOrderBloc;
  int _pageNo = 0;
  bool isListExist = false;

  SalesOrderApprovalListResponse _leaveRequestListResponse;
  ...
  void _applyApprovalStatusList(
      SalesOrderApprovalStatusListResponseState state) {
    ...
    final List<String> nextUpdate = uniqueStatuses.isNotEmpty
        ? uniqueStatuses
    ...
    return BlocProvider(
      ...
      child: BlocConsumer<SalesOrderBloc, SalesOrderStates>(
        ...
        listener: (BuildContext context, SalesOrderStates state) {
          ...
          if (state is SalesOrderSaveResponseState) {
          ...
          if (state is SalesOrderApprovalStatusListResponseState) {
            ...
            _onSalesOrderApprovalStatusListResponseState(state, context);
          }
        },
        listenWhen: (oldState, currentState) {
          return currentState is SalesOrderApprovalListResponseState ||
              currentState is SalesOrderSaveResponseState ||
            ...
            children: [
              ...
              Expanded(
                child: RefreshIndicator(
                  ...
                  child: _buildSalesOrderList(context),
                ),
              ),
            ],
          ),
  ...
  Widget _buildSalesOrderList(BuildContext context) {
    final r = _R(context);
    if (isListExist && _leaveRequestListResponse != null) {
      return NotificationListener<ScrollNotification>(
        ...
        child: ListView.builder(
          ...
          itemBuilder: (ctx, i) => _buildSalesOrderCard(ctx, i),
        ),
      );
    }
    return Center(
  ...
  Widget _buildSalesOrderCard(BuildContext context, int index) {
    final r = _R(context);
    ...
    final String normalizedCurrentStatus = (detail.approvalStatus ?? "").trim();
    final List<String> dropdownStatuses = normalizedCurrentStatus.isEmpty
        ? _updateStatusList
        : (_updateStatusList.contains(normalizedCurrentStatus)
            ? _updateStatusList
            : [normalizedCurrentStatus, ..._updateStatusList]);

    final String dropdownValue = dropdownStatuses
            .contains(normalizedCurrentStatus)
        ? normalizedCurrentStatus
        : (dropdownStatuses.isNotEmpty ? dropdownStatuses.first : "Pending");
  ...
  void _onInquiryListCallSuccess(SalesOrderApprovalListResponseState state) {
  ...
  void _onTapOfEditCustomer(SalesOrderApprovalListResponseDetails detail) {
    ...
    showCommonDialogWithTwoOptions(
      context,
      "Are you sure you want to update Sales Order Approval?",
      negativeButtonTitle: "No",
      positiveButtonTitle: "Yes",
      onTapOfPositiveButton: () {
        Navigator.of(context).pop();
        _salesOrderBloc.add(SalesOrderApprovalSaveRequestEvent(
  ...
  void _onSalesOrderSaveResponseState(
      SalesOrderSaveResponseState state, BuildContext context7up) {
    final message = state.response.toString();
    showCommonDialogWithSingleOption(context7up, message,
  ...
  void _onSalesOrderApprovalStatusListResponseState(
