...
class QuotationReportListScreen extends BaseStatefulWidget {
  static const routeName = '/QuotationReportListScreen';

  @override
  _QuotationReportListScreenState createState() => _QuotationReportListScreenState();
}

class _QuotationReportListScreenState extends BaseState<QuotationReportListScreen>
  ...
  void initState() {
    ...
    _mainBloc.add(QuotationReportListCallEvent(
        1,
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      lazy: true,
      create: (BuildContext context) => _mainBloc..add(QuotationReportListCallEvent(
          1,
          QuotationListApiRequest(
              CompanyId: CompanyID.toString(),
              LoginUserID: LoginUserID,
      ...
      child: BlocConsumer<MainBloc, MainStates>(
        builder: (BuildContext context, MainStates state) {
          if (state is QuotationReportListCallResponseState) {
            _onCustomerReportsResponseSuccess(state);
          }
          return super.build(context);
        },
        buildWhen: (oldState, currentState) {
          if (currentState is QuotationReportListCallResponseState) {
            return true;
          }
          return false;
        },
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        ...
        appBar: NewGradientAppBar(
          title: Text('Quotation Reports Screen'),
  ...
  Widget ExpantionCustomer(BuildContext context, int index) {
    List<QuotationDetails> PD = Response.details;
  ...
  void _displayPdf(List<QuotationDetails> productPaginationDetails) {
      ...
      pw.MultiPage(
        ...
        build: (pw.Context context) => [
          pw.Header(
              ...
              pw.Center(
                child: pw.Text('Quotation Report', style: pw.TextStyle(fontSize: 25, fontWeight: pw.FontWeight.bold)),
              )
          ),
          pw.SizedBox(
              height: 40
          ...
          pw.Table.fromTextArray(
            ...
            headers: ['Sr No.', 'Quotation No' , 'Inquiry No', 'Date', 'Customer', 'Products', 'City' , 'State' ,'Status','Employee','Amount'],
            columnWidths: {
              0: pw.FlexColumnWidth(1),
              1: pw.FlexColumnWidth(2), // Serial Number
              2: pw.FlexColumnWidth(2), // Serial Number
              3: pw.FlexColumnWidth(3), // Serial Number
              ...
              9: pw.FlexColumnWidth(3), // Serial Number// Serial Number
              10: pw.FlexColumnWidth(2), // Serial Number// Serial Number
              11: pw.FlexColumnWidth(3), // Serial Number// Serial Number
            },
            data:List<List<dynamic>>.generate(
              productPaginationDetails.length,
                  (index) => [
                index + 1,
                productPaginationDetails[index].inquiryNo,
                productPaginationDetails[index].inquiryDate.getFormattedDate(
                    fromFormat: "yyyy-MM-dd",
                    toFormat: "dd-MM-yyyy"),
                productPaginationDetails[index].customerName,
                productPaginationDetails[index].,
                productPaginationDetails[index].inquirySource,
                "Ahmedabad",
                "Gujrat",
                productPaginationDetails[index].inquiryStatus,
                productPaginationDetails[index].employeeName,
                productPaginationDetails[index].totalAmount,
    ...
    Navigator.push(context, MaterialPageRoute(builder: (context) => PreviewScreen(doc: doc, PdfName: "productName")));
  }
  ...
  void _onCustomerReportsResponseSuccess(QuotationReportListCallResponseState state) {
