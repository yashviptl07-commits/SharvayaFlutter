...
class LoanListScreen extends BaseStatefulWidget {
...
*/
    ...
    return BlocProvider(
      ...
      child: BlocConsumer<LoanScreenBloc, LoanScreenStates>(
        builder: (BuildContext context, LoanScreenStates state) {
          if (state is LoanListResponseState) {
        ...
        buildWhen: (oldState, currentState) {
          if (currentState is LoanListResponseState ||
              currentState is LoanSearchResponseState ||
        ...
        listenWhen: (oldState, currentState) {
          if (currentState is LoanDeleteResponseState) {
            return true;
          }
          return false;
        },
            ...
            children: [
              Expanded(
                child: RefreshIndicator(
                  onRefresh: () async {
                    _CustomerBloc.add(LoanListCallEvent(
                        1,
                        LoanListRequest(
                          pkID: "",
                          CompanyID: CompanyID.toString(),
  ...
  void _onInquiryListCallSuccess(LoanListResponseState state) {
  ...
  Future<void> _onTapOfSearchView() async {
    navigateTo(context, SearchLoanScreen.routeName).then((value) {
      if (value != null) {
  ...
  void _onCustomerDeleteCallSucess(
      LoanDeleteResponseState state, BuildContext context) {
  ...
  void getUserRights(MenuRightsResponse menuRightsResponse) {
    for (int i = 0; i < menuRightsResponse.details.length; i++) {
      ...
      if (menuRightsResponse.details[i].menuName == "pgLoan") {
