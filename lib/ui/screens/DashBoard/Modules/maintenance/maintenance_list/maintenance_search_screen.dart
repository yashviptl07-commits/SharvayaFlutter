...
class SearchMaintenanceScreen extends BaseStatefulWidget {
  static const routeName = '/SearchMaintenanceScreen';

  @override
  _SearchMaintenanceScreenState createState() =>
      _SearchMaintenanceScreenState();
}

class _SearchMaintenanceScreenState extends BaseState<SearchMaintenanceScreen>
    with BasicScreen, WidgetsBindingObserver {
  MaintenanceScreenBloc _CustomerBloc;
  MaintenanceListResponse _searchCustomerListResponse;
    ...
    return BlocProvider(
      ...
      child: BlocConsumer<MaintenanceScreenBloc, MaintenanceScreenStates>(
        ...
        buildWhen: (oldState, currentState) {
          if (currentState is MaintenanceSearchResponseState) {
            return true;
          }
          return false;
        },
  ...
  Widget _buildSearchInquiryListItem(int index) {
    MaintenanceDetails model = _searchCustomerListResponse.details[index];
  ...
  void _onSearchInquiryListCallSuccess(MaintenanceSearchResponseState state) {
    _searchCustomerListResponse = state.maintenanceListResponse;
  }
}
