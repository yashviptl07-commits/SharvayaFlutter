...
class AcurabathSearchQuotationCustomerScreen extends BaseStatefulWidget {
  static const routeName = '/AcurabathSearchQuotationCustomerScreen';

  @override
  _AcurabathSearchQuotationCustomerScreenState createState() =>
      _AcurabathSearchQuotationCustomerScreenState();
}

class _AcurabathSearchQuotationCustomerScreenState
    extends BaseState<AcurabathSearchQuotationCustomerScreen>
    ...
    return BlocProvider(
      ...
      child: BlocConsumer<QuotationBloc, QuotationStates>(
        builder: (BuildContext context, QuotationStates state) {
          if (state is QuotationCustomerListByNameCallResponseState) {
            _onSearchInquiryListCallSuccess(state);
          }
          return super.build(context);
        },
        buildWhen: (oldState, currentState) {
          if (currentState is QuotationCustomerListByNameCallResponseState) {
            return true;
          }
          return false;
        },
  ...
  Widget buildBody(BuildContext context) {
    return Column(
      children: [
        NewGradientAppBar(
          title: Text('Acurabath Search Customer'),
  ...
  void _onSearchChanged(String value) {
    if (value.trim().length > 2) {
      _inquiryBloc.add(SearchQuotationCustomerListByNameCallEvent(
  ...
  void _onSearchInquiryListCallSuccess(
      QuotationCustomerListByNameCallResponseState state) {
