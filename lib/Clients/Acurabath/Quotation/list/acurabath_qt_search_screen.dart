...
class AcurabathSearchQuotationScreen extends BaseStatefulWidget {
  static const routeName = '/AcurabathSearchQuotationScreen';

  @override
  _AcurabathSearchQuotationScreenState createState() =>
      _AcurabathSearchQuotationScreenState();
}

class _AcurabathSearchQuotationScreenState
    extends BaseState<AcurabathSearchQuotationScreen>
    with BasicScreen, WidgetsBindingObserver {
  QuotationBloc _QuotationBloc;
  SearchQuotationListResponse _searchQuotationListResponse;
  int CompanyID = 0;
  String LoginUserID = "";
    ...
    return BlocProvider(
      ...
      child: BlocConsumer<QuotationBloc, QuotationStates>(
        builder: (BuildContext context, QuotationStates state) {
          if (state is SearchQuotationListByNameCallResponseState) {
            _onSearchInquiryListCallSuccess(state);
          }
          return super.build(context);
        },
        buildWhen: (oldState, currentState) {
          if (currentState is SearchQuotationListByNameCallResponseState) {
            return true;
          }
          return false;
        },
  ...
  Widget buildBody(BuildContext context) {
    return Column(
      children: [
        NewGradientAppBar(
          title: Text('Acurabath Search Quotation'),
  ...
  Widget _buildInquiryList() {
    if (_searchQuotationListResponse == null) {
      return Container();
    }
    return ListView.builder(
      ...
      shrinkWrap: true,
      itemCount: _searchQuotationListResponse.details.length,
    );
  }

  ///builds row item view of inquiry list
  Widget _buildSearchInquiryListItem(int index) {
    SearchDetails model = _searchQuotationListResponse.details[index];
  ...
  void _onSearchChanged(String value) {
    if (value.trim().length > 2) {
      _QuotationBloc.add(SearchQuotationListByNameCallEvent(
          SearchQuotationListByNameRequest(
              word: value,
  ...
  void _onSearchInquiryListCallSuccess(
      SearchQuotationListByNameCallResponseState state) {
    _searchQuotationListResponse = state.response;
  }
}
