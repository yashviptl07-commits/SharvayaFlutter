...
class BTCountryArguments {
  String model123;

  BTCountryArguments(this.model123);
}

class BTSearchCountryScreen extends BaseStatefulWidget {
  static const routeName = '/BTSearchCountryScreen';
  final BTCountryArguments arguments;

  BTSearchCountryScreen(this.arguments);

  @override
  _BTSearchCountryScreen createState() => _BTSearchCountryScreen();
}

class _BTSearchCountryScreen extends BaseState<BTSearchCountryScreen>
  ...
  int CompanyID = 0;
  String LoginUserID = "";
  CountryListResponse _searchCountryListResponse;
    ...
    if (widget.arguments != null) {
      ...
      _CustomerBloc
        ..add(BTCountryListRequestEvent(BTCountryListRequest(
          ...
          CountryCode: widget.arguments.model123,
          ListMode: "L",
        )));
    }
  }
      ...
      builder: (BuildContext context, void Function(void Function()) myState) {
        ...
        return ListView.builder(
          itemBuilder: (context, index) {
            BTCountryListResponseDetails _model =
            ...
            _model.telephonicCode = _listFilteredCountry[index].MenuName;
            return Container(
  ...
  void _onSearchInquiryListCallSuccess(BTCountryListResponseState state) {
    arr_ALL_Name_ID_For_Country.clear();
    _listFilteredCountry.clear();
    for (var i = 0; i < state.btCountryListResponse.details.length; i++) {
      print("CustomerCategoryResponse2 : " +
          state.btCountryListResponse.details[i].countryName);
      ALL_Name_ID categoryResponse123 = ALL_Name_ID();
      categoryResponse123.Name =
          state.btCountryListResponse.details[i].countryName;
      categoryResponse123.Name1 =
          state.btCountryListResponse.details[i].countryCode;

      categoryResponse123.MenuName =
          state.btCountryListResponse.details[i].telephonicCode;
