...
class HplOldAddQuotationProductListArgument {
  String quotation_No;
  String stateCode;
...
class HplOldQuotationProductListScreen extends BaseStatefulWidget {
  static const routeName = '/HplOldQuotationProductListScreen';
  final HplOldAddQuotationProductListArgument arguments;
  HplOldQuotationProductListScreen(this.arguments);
  @override
  _HplOldQuotationProductListScreenState createState() =>
      _HplOldQuotationProductListScreenState();
}

class _HplOldQuotationProductListScreenState
    extends BaseState<HplOldQuotationProductListScreen>
    with BasicScreen, WidgetsBindingObserver {
  List<QuotationTable1> _productList = [];

  List<QuotationSpecificationTable> _quotationProductSpecification = [];

  List<QuotationTable1> _TempinquiryProductList = [];

  List<QuotationTable1> AfterDiscountProductList = [];
    ...
    return BlocProvider(
      ...
      child: BlocConsumer<QuotationBloc, QuotationStates>(
        ...
        listener: (BuildContext context, QuotationStates state) {
          ...
          if (state is QuotationOneProductDeleteState1) {
        ...
        listenWhen: (oldState, currentState) {
          if (currentState is SpecificationListResponseState ||
              currentState is QuotationOneProductDeleteState1 ||
              currentState
      ...
      floatingActionButton: Column(
        ...
        children: [
          FloatingActionButton(
            ...
            onPressed: () async {
              await navigateTo(
                      context, HplOldAddQuotationProductScreen.routeName,
                      arguments: HplOldAddQuotationProductScreenArguments(
                          qtModel, _StateCode, _HeaderDiscAmnt, QuotationNo))
                  .then((value) {
                List<QuotationTable1> temp_productList = value;
  ...
  Future<void> getProduct() async {
    _productList.clear();
    _productList
        .addAll(await OfflineDbHelper.getInstance().getHplQuotationProduct());
  ...
  Future<void> _onTapOfEditContact(int index) async {
    ...
    navigateTo(context, HplOldAddQuotationProductScreen.routeName,
            arguments: HplOldAddQuotationProductScreenArguments(
  ...
  _onTapOfDeleteContact(int id, int ItemIndex) {
    ...
    _inquiryBloc.add(QuotationOneProductDeleteEvent1(id));
  ...
  ExpantionCustomer(BuildContext context, int index) {
    QuotationTable1 model = _productList[index];
  ...
  void _ontaptoSpecificationADDEdit(QuotationTable1 model) {}
  ...
  void _onQuotationOneProductDeleteResponse(
      QuotationOneProductDeleteState1 state) {
